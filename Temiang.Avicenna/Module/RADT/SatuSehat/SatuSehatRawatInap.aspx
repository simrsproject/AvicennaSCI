<%@ Page Language="C#" MasterPageFile="~/MasterPage/MasterCustom.Master" AutoEventWireup="true"
    CodeBehind="SatuSehatRawatInap.aspx.cs" Inherits="Temiang.Avicenna.Module.RADT.SatuSehatRawatInap" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <telerik:RadCodeBlock ID="RadCodeBlock1" runat="server">
        ...
        <script type="text/javascript" language="javascript">
            ...
            function openDetail(regNo) {
                var oWnd = $find("<%= winDetail.ClientID %>");
                var url = 'SatuSehatILPDetail.aspx?mod=view&templateid=100&regno=' + regNo;
                // JANGAN PAKAI radopen,  urlnya harus lengkap dgn rootnya jika pakai radopen
                oWnd.setUrl(url);
                oWnd.setSize(1040, 600);
                //oWnd.maximize();
            ...
            function CloseStatus(registrationNo, isClosed) {
                let msg = (isClosed === 1 ? "Close this pending SatuSehat data?" : "Re-open this SatuSehat data?");
                if (confirm(msg)) {
                    __doPostBack("<%= grdRegisteredList.UniqueID %>", "closestatus|" + registrationNo + "|" + isClosed);
                }
            }
    ...
    <telerik:RadWindow ID="winDetail" Width="1000px" Height="900px" runat="server" Modal="true" VisibleStatusbar="true"
        DestroyOnClose="false" Behavior="Close, Move, Maximize" ReloadOnShow="True" ShowContentDuringLoad="false">
    </telerik:RadWindow>
        ...
        <MasterTableView DataKeyNames="RegistrationNo,EncounterID" GroupLoadMode="Server">
            <Columns>
                <telerik:GridTemplateColumn UniqueName="CheckBoxTemplateColumn" HeaderStyle-Width="40px">
                    ...
                    <ItemTemplate>
                        <asp:CheckBox ID="detailChkbox" runat="server"
                            Visible='<%# (Convert.ToBoolean(Eval("IsAllowProcess"))) %>'>
                        </asp:CheckBox>                  
                    </ItemTemplate>
                </telerik:GridTemplateColumn>
                <telerik:GridTemplateColumn HeaderText="" UniqueName="btnEdit">
                    <HeaderStyle HorizontalAlign="Left" Width="40px" />
                    <ItemStyle HorizontalAlign="Left" />
                    <ItemTemplate>
                        <%# string.Format("<a style=\"cursor: pointer;\" onclick=\"openDetail('{0}')\"><img src=\"../../../Images/Toolbar/edit16.png\" border=\"0\" alt=\"Detail\" title=\"Detail\" /></a>", DataBinder.Eval(Container.DataItem, "RegistrationNo"))%>
                    </ItemTemplate>
                </telerik:GridTemplateColumn>
                ...
                <telerik:GridCheckBoxColumn DataField="IsSoap" HeaderText="Soap"
                    UniqueName="IsSoap" SortExpression="IsSoap" Visible="false">
                    <HeaderStyle HorizontalAlign="Center" Width="50px" />
                    <ItemStyle HorizontalAlign="Center" />
                </telerik:GridCheckBoxColumn>
                <telerik:GridCheckBoxColumn DataField="IsIcd10" HeaderText="ICD-10"
                    UniqueName="IsIcd10" SortExpression="IsIcd10" Visible="false">
                    <HeaderStyle HorizontalAlign="Center" Width="50px" />
                    <ItemStyle HorizontalAlign="Center" />
                </telerik:GridCheckBoxColumn>
                <telerik:GridCheckBoxColumn DataField="IsVitalSign" HeaderText="VSign"
                    UniqueName="IsVitalSign" SortExpression="IsVitalSign" Visible="false">
                    <HeaderStyle HorizontalAlign="Center" Width="50px" />
                    <ItemStyle HorizontalAlign="Center" />
                </telerik:GridCheckBoxColumn>
                <telerik:GridCheckBoxColumn DataField="IsIcd9" HeaderText="ICD-9"
                    UniqueName="IsIcd9" SortExpression="IsIcd9" Visible="false">
                    <HeaderStyle HorizontalAlign="Center" Width="50px" />
                    <ItemStyle HorizontalAlign="Center" />
                </telerik:GridCheckBoxColumn>
                <telerik:GridCheckBoxColumn DataField="IsEduDiet" HeaderText="Diet"
                    UniqueName="IsEduDiet" SortExpression="IsEduDiet" Visible="false">
                    <HeaderStyle HorizontalAlign="Center" Width="50px" />
                    <ItemStyle HorizontalAlign="Center" />
                </telerik:GridCheckBoxColumn>
                <telerik:GridCheckBoxColumn DataField="IsPrescription" HeaderText="Presc."
                    UniqueName="IsPrescription" SortExpression="IsPrescription" Visible="false">
                    <HeaderStyle HorizontalAlign="Center" Width="50px" />
                    <ItemStyle HorizontalAlign="Center" />
                </telerik:GridCheckBoxColumn>
                <telerik:GridCheckBoxColumn DataField="IsGenCons" HeaderText="Gen. Consent"
                    UniqueName="IsGenCons" SortExpression="IsGenCons" Visible="false">
                    <HeaderStyle HorizontalAlign="Center" Width="50px" />
                    <ItemStyle HorizontalAlign="Center" />
                </telerik:GridCheckBoxColumn>
                ...
                <telerik:GridTemplateColumn Groupable="false" HeaderStyle-HorizontalAlign="Center"
                    ...
                    <ItemTemplate>
                        <%# string.Format(
                            "<a href=\"#\" onclick=\"CloseStatus('{0}', {1}); return false;\">{2}</a>",
                            DataBinder.Eval(Container.DataItem, "RegistrationNo"),
                            (
                                (DataBinder.Eval(Container.DataItem, "IsClosed") == DBNull.Value 
                                 || string.IsNullOrEmpty(DataBinder.Eval(Container.DataItem, "IsClosed").ToString()))
                                    ? "1" // kalau NULL / kosong → dianggap false → kasih param 1 (close)
                                    : (Convert.ToBoolean(DataBinder.Eval(Container.DataItem, "IsClosed")) ? "0" : "1")
                            ),
                            (
                                (DataBinder.Eval(Container.DataItem, "IsClosed") == DBNull.Value 
                                 || string.IsNullOrEmpty(DataBinder.Eval(Container.DataItem, "IsClosed").ToString()))
                                    ? "<img src='../../../Images/Toolbar/close.png' border='0' title='Close' />"
                                    : (Convert.ToBoolean(DataBinder.Eval(Container.DataItem, "IsClosed"))
                                        ? "<img src='../../../Images/Toolbar/open.png' border='0' title='Open' />"
                                        : "<img src='../../../Images/Toolbar/close.png' border='0' title='Close' />")
                            )
                        ) %>
                    </ItemTemplate>
