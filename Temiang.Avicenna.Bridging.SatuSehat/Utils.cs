...
namespace Temiang.Avicenna.Bridging.SatuSehat
{
    ...
    public class Utils : Temiang.Avicenna.Bridging.SatuSehat.BusinessObject.BaseUtil
    {
        ...
        public void PostServiceRequestRad(Registration reg, PatientBridging patSs, ParamedicBridging parMedSs, string encounterId, ref string accessToken)
        {
            ...
            foreach (DataRow row in dtb.Rows)
            {
                ...
                itemSs.Query.Where(
                    ...
                    itemSs.Query.SRBridgingType == _satuSehatBridgingType
                );
                itemSs.Query.es.Top = 1;
            ...
            var postData = new
            {
                ...
                new {
                    ...
                    type = new {
                        ...
                        system = $"http://sys-ids.kemkes.go.id/acsn/{_organizationID}",
                ...
                encounter = new
                {
                    ...
                    display = $"Permintaan {loincItem.Display} {patSs.BridgingName} di hari {_dayNames[reg.RegistrationDate.Value.DayOfWeek.ToInt()]} pukul {approvedDateTime.AddHours(_gmtDif):HH:mm} WIB"
                },
                occurrenceDateTime = $"{approvedDateTime.AddHours(_gmtDif):yyyy-MM-ddTHH:mm:ss}+00:00",
                requester = new
                {
        ...
        private ImmunizationPost.Root ImmunizationPostData(ref SatuSehatResult ssResult, Registration reg, PatientBridging patSs, ParamedicBridging parMedSs, string encounterId, PatientImmunization pim)
        {
            ...
            if (string.IsNullOrEmpty(ipm.SRRoute))
            {
                ssResult.ErrorResponse = string.Format("Route for Item Product Medical Item ID: {0} still empty", ipm.ItemID);
                return null;
            }
